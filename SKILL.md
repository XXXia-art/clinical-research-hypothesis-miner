...
- 动作：
  ...
  - 只要执行检索或接收用户材料，就必须形成可保存的文件级记录，建议命名为 `evidence-packet-[topic-slug]-[YYYYMMDD].md`；不得只在对话中写“检索完成”。
...
- 使用 reference：[科研总结报告装配模板](references/research-summary-assembly.md)。
- 输入：Phase 1-6 已经生成的报告片段和阶段产物。
- 动作：只装配已有片段，生成可保存、可下载、可追溯的 Markdown 科研总结报告；不额外思考，不重新分析，不补充前序阶段没有给出的判断。
- 输出：科研总结报告。
...
| 科研总结报告装配输出 | [research-summary-assembly.md](references/research-summary-assembly.md) |
