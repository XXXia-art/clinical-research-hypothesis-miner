...
如果用户已经使用本 Skill 完成过一轮分析，并且已经生成 Phase 7 科研总结报告，则后续多轮对话不得默认从 Phase 1 重新开始。

每次用户提出后续问题时，先执行以下步骤：

1. 回到最近一次 Phase 7 科研总结报告，确认报告中的研究问题、证据来源、创新地图、空白审计、候选假说和验证路径。
2. 判断用户新问题对应报告中的哪个章节，以及属于哪一个 Phase 的管理范围。
3. 从对应 Phase 切入，只更新该问题影响到的阶段产物。
4. 如果该问题会影响下游阶段，说明需要级联更新哪些 Phase。
5. 不得在没有回溯报告和上游阶段产物的情况下，直接生成新的判断、假说或结论。

### 后续问题到 Phase 的路由表

| 用户后续问题类型 | 优先回到报告章节 | 切入 Phase |
|---|---|---|
| 想修改研究对象、疾病、机制、终点、关键词 | 研究问题与边界 | Phase 1 |
| 想补充新论文、摘要、PMID、DOI、网页、数据集 | 证据来源与材料范围 | Phase 2 |
| 想问当前文献覆盖了什么、缺什么、证据分布如何 | 文献与资源证据概述 | Phase 3 |
| 想问哪里可能有创新点、哪些方向拥挤或薄弱 | 创新点证据地图 | Phase 4 |
| 想质疑某个空白是否真实、是否过度推断 | 研究空白审计结果 | Phase 5 |
| 想细化假说、验证路径、实验方案、Lite/Standard/Publication+ | 候选假说与验证路径 | Phase 6 |
| 想重写、压缩、导出、更新最终报告 | 全部报告结构 | Phase 7 |

### 级联更新规则

- 如果从 Phase 1 更新研究问题，通常需要重新检查 Phase 2-7。
- 如果从 Phase 2 补充或替换证据，需要更新 Phase 3-7。
- 如果从 Phase 3 调整证据概述，需要更新 Phase 4-7。
- 如果从 Phase 4 调整创新地图，需要更新 Phase 5-7。
- 如果从 Phase 5 调整空白审计，需要更新 Phase 6-7。
- 如果从 Phase 6 调整假说或验证路径，只需更新 Phase 7。
- 如果只是 Phase 7 的表达、格式或报告装配修改，不应重新分析前序阶段。
